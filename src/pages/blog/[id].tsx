...
const components: Pick<PortableTextReactComponents, 'block' | 'list' | 'types'> = {
    block: {
        ...
        blockquote: ({children}) => <blockquote
            className='border-l-gray-600 dark:border-l-gray-400 border-solid border-l-2 pl-2 my-2 whitespace-pre-wrap'>{children}</blockquote>,
    },
    list: {
    ...
    types: {
        image: ({value}) => <img src={value.assetUrl} alt='blog image'/>,
        file: ({value}) => {
            return <div className='w-full'>
                <video controls={true} className='object-fill'>
                    <source src={value.assetUrl}/>
                </video>
            </div>
        },
        remoteVideo: ({value}) => {
            return <div className='w-full flex justify-center'>
                <iframe src={value.url} width='640' height='360'/>
            </div>
        }
    }
}
            ...
            <header className='mb-4 lg:mb-6'>
                ...
                <div className='inline-flex items-center mr-3 text-sm text-gray-900 dark:text-white'>
                    <img src={post.author.image} alt={post.author.name}
                         className='h-16 w-16 rounded-full bg-white/10 mr-8'/>
                    <div>
